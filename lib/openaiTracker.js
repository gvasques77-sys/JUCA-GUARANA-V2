...
// Registra tokens, custo, latência e clinic_id na tabela `clinic_ai_usage`
// (tabela pré-existente do F8 backoffice, estendida via migration Sprint 0
//  `sprint0_010_extend_clinic_ai_usage` com as colunas cached_tokens, latency_ms,
...
// - `usage_type` DEVE ser um dos 4 valores do CHECK legado
//   ('conversation' | 'task_processing' | 'report' | 'other').
//   Usamos deriveUsageType(purpose) para mapear purpose granular → usage_type legado.
//   Isso garante que o F8 (que agrega por usage_type) continue funcionando
//   enquanto a Sprint 0+ consome a coluna nova `purpose` para granularidade.
//
...
/**
 * Mapeia um `purpose` granular (Sprint 0+) para um dos 4 valores do CHECK
 * constraint legado em clinic_ai_usage.usage_type. NÃO alterar o CHECK —
 * este mapeamento é a ponte de compatibilidade com o F8 backoffice.
 *
 * Valores legais:
 *   'conversation' | 'task_processing' | 'report' | 'other'
 */
export function deriveUsageType(purpose) {
  const p = String(purpose || '');
  // Lara/WhatsApp → conversation
  if (p.startsWith('lara_')) return 'conversation';
  // CRM relatórios → report
  if (p.startsWith('crm_report') || p.startsWith('crm_overview')) return 'report';
  // Prontuário lara-assist → other (sem categoria específica no CHECK legado)
  if (p.startsWith('prontuario_')) return 'other';
  // Task processor worker → task_processing
  if (p.startsWith('task_')) return 'task_processing';
  // fallback seguro
  return 'other';
}

/**
 * Grava um registro em clinic_ai_usage fire-and-forget.
 * Nunca lança — falhas são apenas logadas.
 *
 * NÃO incluir `tokens_total` no payload — é coluna GENERATED ALWAYS AS.
 */
...
export async function trackedChatCompletion({
  ...
  // Mapeamento para clinic_ai_usage:
  //   tokens_input  ← prompt_tokens
  //   tokens_output ← completion_tokens
  //   cost_usd      ← calculateCost(...)
  //   usage_type    ← deriveUsageType(purpose) — obrigatório pelo CHECK legado
  //   tokens_total  NÃO incluído (coluna GENERATED)
  logUsageFireAndForget({
    clinic_id: clinicId,
    usage_type: deriveUsageType(purpose),
    model: modelUsed,
    purpose,
    tokens_input: promptTokens,
    tokens_output: completionTokens,
    cached_tokens: cachedTokens,
    cost_usd: calculateCost(modelUsed, promptTokens, cachedTokens, completionTokens),
